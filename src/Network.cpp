...
void Network::teach(vector<std::string>& fileNames) {
	...
	while (it != fileNames.end()) {//przejscie po wszystkich plikach wzorcow
		...
		cout << "Wzorzec zapamietany w sieci: " << endl; showNeurons(); cout << endl;
...
void Network::recognize(std::string& fileName) {
	...
	cout <<endl<<endl<<endl<< "Obraz do rozpoznania:" << endl;
...
void Network::recognizingAlg() {
	...
	cout << "\n\n\n\nEtap rozpoznawania wzorca: " << endl;
	while (check) {
		check = false;
		for (int i = 0; i < size; i++) {
			sila = 0;
	...
	std::cout << endl << endl << endl<<"Wzorzec rozpoznany przez siec: " << std::endl;
	showNeurons();
}
